...
//ha a parancssori arhumentumok rosszul letek megadva, akkor is kiíródik az üzenet, de ekkor az stderr-re.    
...
int readConfigParameters(int argc, char **argv, float & sigma_s, float & sigma_r, int & r, int & threads)
{
	...
	//Ellenõrizzük a paraméterek számát. Az utoldó két paraméter, r és threads opcionálisak.
	if (!(argc >= 5 && argc <= 7)) {
